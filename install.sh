...
select_s3_bucket() {
    ...
    if [[ ${#buckets[@]} -eq 0 ]]; then
        echo "⚠️ 未能自动拉取 Bucket 列表，可能是权限不足或兼容服务不支持 ListBuckets。"
...
show_s3_buckets_for_current_account() {
    local buckets=()
    local i bucket
...
select_s3_bucket_for_account() {
    local var_name="$1"
    local current="${2:-}"
    select_s3_bucket "$var_name" "$current"
}
...
add_s3_account() {
    ...
    echo "✅ 账号已保存：$ACCOUNT_ID。使用备份/浏览功能时会自动拉取存储桶列表供你选择。"
    show_s3_buckets_for_current_account || true
    pause
}
...
show_s3_accounts() {
    ...
    for f in "${files[@]}"; do
        # shellcheck disable=SC1090
        source "$f"
        echo "[$i] 👤 账号：$ACCOUNT_ID | 🌐 Region：${S3_REGION:-us-east-1} | 🔗 Endpoint：${S3_ENDPOINT_URL:-AWS默认}"
        i=$((i + 1))
    done
...
delete_s3_account() {
    ...
    read -rp "⚠️ 确认删除账号 [$target_id] 以及它的定时任务吗？(y/N)： " yn
    case "$yn" in
        y|Y)
            rm -f "$file"
...
browse_s3_with_account() {
    ...
    load_s3_account "$ACCOUNT_ID" || { pause; return; }
    select_s3_bucket_for_account S3_BUCKET "${S3_BUCKET:-}" || { pause; return; }
    S3_PREFIX=""
    ...
    while true; do
        ...
        echo "🔍 S3 远程浏览 / 下载 / 删除"
        echo "======================================="
        echo "👤 当前账号：$ACCOUNT_ID | 🪣 当前存储桶：$S3_BUCKET"
        echo
        echo "1) 📋 列出某个目录内容"
        echo "2) 📥 下载文件到本地"
        echo "3) 📥 下载目录到本地"
        ...
        case "$sub" in
            ...
            2)
                ...
                read -rp "⚠️ 确认下载 $src_uri 到 $LDIR 并覆盖同名文件吗？(y/N)： " yn_dl
                case "$yn_dl" in
                    y|Y)
            ...
            3)
                ...
                read -rp "⚠️ 确认同步下载整个目录 $dir_uri 到本地 $LDIR 吗？(y/N)： " yn_dir
                case "$yn_dir" in
                    y|Y)
            ...
            4)
                ...
                read -rp "⚠️ 确认删除远程文件 $file_uri 吗？此操作不可恢复！(y/N)： " yn
                case "$yn" in
                    y|Y)
            ...
            5)
                ...
                read -rp "⚠️ 确认删除整个目录 $rm_uri 吗？此操作不可恢复！(y/N)： " yn2
                case "$yn2" in
                    y|Y)
                ...
                ;;
            6)
                select_s3_bucket_for_account S3_BUCKET "$S3_BUCKET" || true
                pause
                ;;
            0)
                break
...
edit_s3_account() {
    ...
    while true; do
        ...
        echo "======================================="
        echo "[1] 🌐 Region：$S3_REGION"
        echo "[2] 🔗 Endpoint URL：${S3_ENDPOINT_URL:-AWS默认}"
        echo "[3] 🔑 Access Key ID：$S3_ACCESS_KEY"
        echo "[4] 🔒 Secret Access Key：已隐藏"
        echo "[5] 🪣 测试/查看存储桶列表"
        echo "[6] ✅ 保存并退出"
        echo "[0] ⬅ 不保存退出"
        echo
...
run_backup() {
    local ACCOUNT_ID="$1"
    local LOCAL_PATH="$2"
    local S3_BUCKET_ARG="$3"
    local REMOTE_DIR="$4"

    load_s3_account "$ACCOUNT_ID" || return 1

    if [[ -z "$REMOTE_DIR" ]]; then
        REMOTE_DIR="$S3_BUCKET_ARG"
        S3_BUCKET="${S3_BUCKET:-}"
    else
...
add_cron_job() {
    ...
    bucket_q="$(shell_quote "$S3_BUCKET")"
...
add_backup_job() {
    ...
    local ACCOUNT_ID="$CHOSEN_ACCOUNT_ID"
    load_s3_account "$ACCOUNT_ID" || { pause; return; }
    select_s3_bucket_for_account S3_BUCKET "" || { pause; return; }
    S3_PREFIX=""

    echo
    echo "👤 当前目标账号：$ACCOUNT_ID"
    echo "🪣 当前存储桶：$S3_BUCKET"
    ...
    if [[ -z "$run_now" || "$run_now" =~ ^[Yy]$ ]]; then
        run_backup "$ACCOUNT_ID" "$LOCAL_PATH" "$S3_BUCKET" "$REMOTE_DIR"
    fi
...
show_menu() {
    ...
    echo "🧭 层级逻辑：账号 -> 序号选择存储桶 -> 存储桶内目录/文件"
